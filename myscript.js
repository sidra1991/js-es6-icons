...
for (let i = 0; i < icons.length; i++) {
    if (select.includes(icons[i].type) == false ){
        select.push(icons[i].type)
    }
}

for (let i = 0; i < select.length; i++) {
    document.getElementById(`iconsOrder`).innerHTML += `<option value="${select[i]}"> ${select[i]} </option> `; 
}
function dragon() {
    container.innerHTML="";
    let iconaSelection = document.getElementById(`iconsOrder`).value;
    for (let i = 0; i < icons.length; i++) {
        

        if (iconaSelection === "all" ) {
            let name  = icons[i].name; 
            let color = icons[i].color;
            let icona = `${icons[i].prefix}${icons[i].name} ${icons[i].family}`
            container.innerHTML += ` 
            <div class="card"> 

            <i class="${ icona }" style="color:${color}"  ></i>
                ${ name} 
            
            </div> `
        }else
        if ( iconaSelection === icons[i].type ) {
            let name  = icons[i].name; 
            let color = icons[i].color;
            let icona = `${icons[i].prefix}${icons[i].name} ${icons[i].family}`
            container.innerHTML += ` 
